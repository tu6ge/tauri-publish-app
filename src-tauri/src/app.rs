    ...
    pub async fn check_oss(&self) -> Result<HashMap<String, bool>, String> {
        ...
        for entry in path.read_dir().map_err(|e| e.to_string())? {
            ...
            let file_name = entry
                .file_name()
                .into_string()
                .map_err(|_| "file name into string failed".to_owned())?;
...
impl AppList {
    pub fn get_all() -> Result<Self, String> {
        ...
        let mut file = File::options()
            ...
            .read(true)
            .create(true)
            .open(data_dir.unwrap().join("tauri_oss_app_config.json"))
            .map_err(|_| "open tauri_oss_app_config.json failed".to_string())?;

        file.read_to_string(&mut content)
            .map_err(|_| "reading tauri_oss_app_config.json failed".to_string())?;
    ...
    fn save(&self) -> Result<String, String> {
        let json =
            serde_json::to_string_pretty(self).map_err(|_| "转 json 格式失败".to_string())?;
        let data_dir = data_dir();
        if let None = data_dir {
            return Err("找不到存储路径".into());
        }

        let mut file = File::create(data_dir.unwrap().join("tauri_oss_app_config.json"))
            .map_err(|_| "create tauri_oss_app_config.json failed".to_string())?;
        file.write_fmt(format_args!("{}", json))
            .map_err(|_| "writing tauri_oss_app_config.json failed".to_string())?;
